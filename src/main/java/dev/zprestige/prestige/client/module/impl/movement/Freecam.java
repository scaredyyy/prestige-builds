...
public class Freecam extends Module {
    ...
    public void event(TickEvent event) {
        ...
        if (PojavInput.isKeyPressed(((IKeyBinding)keyBinding).getBoundKey().getCode())) {
            vec3d5 = vec3d5.add(vec3d);
        }
        KeyBinding keyBinding2 = getMc().options.backKey;
        if (PojavInput.isKeyPressed(((IKeyBinding)keyBinding2).getBoundKey().getCode())) {
            vec3d5 = vec3d5.subtract(vec3d);
        }
        KeyBinding keyBinding3 = getMc().options.leftKey;
        if (PojavInput.isKeyPressed(((IKeyBinding)keyBinding3).getBoundKey().getCode())) {
            vec3d5 = vec3d5.add(vec3d3);
        }
        KeyBinding keyBinding4 = getMc().options.rightKey;
        if (PojavInput.isKeyPressed(((IKeyBinding)keyBinding4).getBoundKey().getCode())) {
            vec3d5 = vec3d5.add(vec3d4);
        }
        KeyBinding keyBinding5 = getMc().options.jumpKey;
        if (PojavInput.isKeyPressed(((IKeyBinding)keyBinding5).getBoundKey().getCode())) {
            vec3d5 = vec3d5.add(0.0, speed.getObject(), 0.0);
        }
        KeyBinding keyBinding6 = getMc().options.sneakKey;
        if (PojavInput.isKeyPressed(((IKeyBinding)keyBinding6).getBoundKey().getCode())) {
            vec3d5 = vec3d5.add(0.0, -speed.getObject(), 0.0);
        }
        KeyBinding keyBinding7 = getMc().options.sprintKey;
        vec3d5 = vec3d5.normalize().multiply(speed.getObject() * (PojavInput.isKeyPressed(((IKeyBinding)keyBinding7).getBoundKey().getCode()) ? 2 : 1));
        pos = pos2;
    ...
    public void event(@NotNull CameraOffsetEvent event) {
        float f = this.getMc().getRenderTickCounter().getTickDelta(true);
