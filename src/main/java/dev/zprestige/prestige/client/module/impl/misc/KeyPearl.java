...
public class KeyPearl extends Module {
    ...
    public void onEnable() {
        ...
        this.originalSlot = getMc().player == null ? -1 : getMc().player.getInventory().selectedSlot;
        this.reset();
    }

    @EventListener
    public void event(TickEvent event) {
        if (getMc().player == null || getMc().interactionManager == null || getMc().currentScreen != null
                || !dev.zprestige.prestige.client.util.impl.PojavInput.isGameFocused() || !timer.delay(Math.max(125.0f, delay.getValue()))) return;
        if (throwPearl) {
            InventoryUtil.INSTANCE.setCurrentSlot(originalSlot >= 0 ? originalSlot : switchSlot.getObject() - 1);
            toggle();
            return;
        }
    ...
    public void onDisable() {
        if (throwPearl && getMc().player != null && originalSlot >= 0) {
            InventoryUtil.INSTANCE.setCurrentSlot(originalSlot);
        }
        throwPearl = false;
        originalSlot = -1;
    }
