...
public class AntiResourcePack extends Module {
    ...
    public void event(PacketReceiveEvent event) {
        if (event.getPacket() instanceof ResourcePackSendS2CPacket) {
            event.setCancelled();
            PacketUtil.INSTANCE.sendPacket(new ResourcePackStatusC2SPacket(getMc().player.getUuid(), ResourcePackStatusC2SPacket.Status.ACCEPTED));
            PacketUtil.INSTANCE.sendPacket(new ResourcePackStatusC2SPacket(getMc().player.getUuid(), ResourcePackStatusC2SPacket.Status.SUCCESSFULLY_LOADED));
        }
    }
}
