...
public class ESP extends Module {
        ...
        for (PlayerEntity player : getMc().world.getPlayers()) {
            ...
            if (antiBotManager.isNotBot(player)) {
                ...
                if (Prestige.Companion.getSocialsManager().isFriend(string)) {
                    ...
                    color = Prestige.Companion.getSocialsManager().isEnemy(player.getName().getString()) ? new Color(255, 0, 0) : new Color(themeColor.getRed(), themeColor.getGreen(), themeColor.getBlue(), 100);
                }
    ...
    private void renderBrokenBlocks(MatrixStack matrixStack, Vec3d vec3d) {
        ...
        if (quaternionf != null) {
            Int2ObjectMap<BlockBreakingInfo> int2ObjectMap;
            try {
                int2ObjectMap = ((IWorldRenderer)getMc().worldRenderer).getBlockBreakingInfos();
                ...
                if (Math.sqrt(getMc().player.squaredDistanceTo(blockPos.toCenterPos())) < radius.getObject()) {
                    ...
                    if (f > 0.0f) {
                        ...
                        if (entity != null) {
                            ...
                            String string = entity.getName().getString() + " " + (int) (f * 100) + "%";
                            font.drawString(entity.getName().getString(), -font.getStringWidth(string) / 2, -font.getStringHeight() / 2, themeColor);
                            String string2 = (int) (f * 100) + "%";
                            font.drawString(string2, -font.getStringWidth(string) / 2 + font.getStringWidth(entity.getName().getString()) / 2 - font.getStringWidth(string2) / 2, font.getStringHeight() / 2, themeColor);
                        }
    ...
    private void renderPearlTrajectories(Vec3d vec3d) {
        ...
        var entityIterator = entities.entrySet().iterator();
        while (entityIterator.hasNext()) {
            Map.Entry<Entity, Map<Long, Vec3d>> entry = entityIterator.next();
            entry.getValue().entrySet().removeIf(point -> System.currentTimeMillis() - point.getKey() > 2000L);
            if (entry.getValue().isEmpty() && !entry.getKey().isAlive()) {
    ...
    private void renderStorages(Vec3d vec3d) {
        RenderUtil.setCameraAction();
        java.util.List<BlockEntityTickInvoker> tickers;
        try {
            tickers = ((IWorld)getMc().world).getBlockEntityTickers();
        } catch (Throwable ignored) {
            RenderHelper.getMatrixStack().pop();
            return;
        }
        for (BlockEntityTickInvoker blockEntityTickInvoker : tickers) {
    ...
    private void setMatrix(MatrixStack matrixStack) {
        ...
        if (entity instanceof PlayerEntity player) {
            float f = getMc().getRenderTickCounter().getTickDelta(true);
