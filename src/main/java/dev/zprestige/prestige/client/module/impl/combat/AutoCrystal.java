...
public class AutoCrystal extends Module {
    ...
    public TimerUtil androidActionTimer;
    public BlockPos pos;
    ...
    public AutoCrystal() {
        ...
        timer = new TimerUtil();
        androidActionTimer = new TimerUtil();
    }
    ...
    public void event(MoveEvent event) {
        if (getMc().currentScreen != null || !PojavInput.isGameFocused() || pauseOnKill.getObject() && OneLineUtil.isInvalidPlayer()) {
            return;
        }

        if (PojavInput.isPojav() && !androidActionTimer.delay(175.0f)) {
            return;
        }
        ...
        timer.reset();
        delay.setValue();
        if (PojavInput.isPojav()) androidActionTimer.reset();
        ...
        if (bool) {
            ...
            } else if (headBob.getObject() && !PojavInput.isPojav()) {
        ...
        if (hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            if (getMc().world.getBlockState(blockHitResult.getBlockPos()).getBlock() == Blocks.BEDROCK || getMc().world.getBlockState(blockHitResult.getBlockPos()).getBlock() == Blocks.OBSIDIAN) {
                ...
                if (InventoryUtil.INSTANCE.isHoldingItem(Items.END_CRYSTAL) || silentSwap.getObject()) {
                    ...
                    Integer n2 = InventoryUtil.INSTANCE.findItemInHotbar(Items.END_CRYSTAL);
                    if (!InventoryUtil.INSTANCE.isHoldingItem(Items.END_CRYSTAL)
                            && n2 != null && silentSwap.getObject()) {
                        getMc().player.getInventory().selectedSlot = n2;
                        return; // Let Minecraft send selected-slot update before place packet.
                    }
                    ClientPlayerInteractionManager clientPlayerInteractionManager = getMc().interactionManager;
