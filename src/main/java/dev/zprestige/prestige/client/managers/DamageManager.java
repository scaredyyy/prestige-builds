...
public class DamageManager implements MC {
    ...
    public double crystalDamage(PlayerEntity player, Vec3d crystal, boolean predictMovement, BlockPos obsidianPos, boolean ignoreTerrain) {
        ...
        damage = DamageUtil.getDamageLeft(player, (float) damage, getMc().world.getDamageSources().explosion(explosion), (float) player.getArmor(), (float) player.getAttributeInstance(EntityAttributes.GENERIC_ARMOR_TOUGHNESS).getValue());
    ...
    private double getExposure(Vec3d source, Entity entity, boolean predictMovement, BlockPos obsidianPos, boolean ignoreTerrain) {
    ...
    private double blastProtReduction(LivingEntity player, double damage, Explosion explosion) {
        int protLevel = 0;
    ...
    private double normalProtReduction(LivingEntity player, double damage) {
        int protLevel = 0;
        if (protLevel > 20) protLevel = 20;
