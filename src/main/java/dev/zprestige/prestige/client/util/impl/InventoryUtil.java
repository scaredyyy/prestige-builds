...
public class InventoryUtil implements MC {
    ...
    public boolean hasStatusEffect(ItemStack itemStack, RegistryEntry<StatusEffect> statusEffect) {
        for (StatusEffectInstance statusEffectInstance : itemStack.getOrDefault(DataComponentTypes.POTION_CONTENTS, PotionContentsComponent.DEFAULT).getEffects()) {
            if (statusEffectInstance.getEffectType().equals(statusEffect)) {
                return true;
            }
        }
        return false;
