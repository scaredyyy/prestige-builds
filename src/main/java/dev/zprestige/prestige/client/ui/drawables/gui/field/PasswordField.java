...
public class PasswordField {
    ...
    public void render(int n, int n2, float f, boolean show) {
        render(n, n2, f, 0.8f, show);
    }

    public void render(int n, int n2, float f, float f2, boolean show) {
        RenderUtil.renderColoredQuad(x, y, x + width, y + height, RenderUtil.getColor(-2, f));
        RenderUtil.renderRoundedRectOutline(x, y, x + width, y + height, RenderUtil.getColor(-5, f), 0.0f);
    ...
    public void keyPressed(int n) {
        if (active) {
            ...
            switch (n) {
                case 86 -> {
                    if (PojavInput.isKeyPressed(341)) {
                        try {
